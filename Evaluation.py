...
from Evaluation_modules.evaluate_model import evaluate_model, evaluate_model_overlapping_patches_return_dict
...
def evaluate_model_on_given_dataset_with_overlapping_patches(mode, model_config, data_config, complete_test_directory,
                                                             data_parent_dir):
    if mode == "test":
        ground_truth_dir = os.path.join(data_parent_dir, 'data_raw', 'zones', "test")
    else:
        ground_truth_dir = os.path.join(data_parent_dir, 'data_raw_val_as_test_set', 'zones', "test")
    ...
    whole_dict = evaluate_model_overlapping_patches_return_dict(model, loader, complete_test_directory,
                                                                ground_truth_dir)
        ...
        if save_images:
            Image.fromarray(post_complete_predicted_mask.astype(np.uint8)).save(
                os.path.join(complete_postprocessed_directory,
                             file_name + ".png"))  # Image.av(os.path.join(complete_postprocessed_directory, file_name), post_complete_predicted_mask)
...
def start_post_process(mode, names, data_config, model_config,
                       complete_test_directory, complete_postprocessed_directory, visualizations_dir,
    ...
    if load_results_from_file:
        ...
        if hparams.with_overlap == True:
            complete_predicted_dict = evaluate_model_on_given_dataset_with_overlapping_patches(mode, model_config,
                                                                                               data_config,
    ...
    post_processed_dictionary = post_processing(names, complete_predicted_dict, bounding_boxes_directory,
                                                complete_postprocessed_directory)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', default="val", help="Either 'val' or 'test'.")
    parser.add_argument('--data_raw_parent_dir', default=r"..",
                        help="Where the data_raw directory lies, default: .")
    parser.add_argument('--with_overlap', action=argparse.BooleanOptionalAction, default=False)
    parser.add_argument('--experiment_name', default="testtest",
    ...
    print(f"Overlap: {hparams.with_overlap}")
    if hparams.mode == "test":
        if hparams.with_overlap:
            hparams.data_config = "test.yaml"
        else:
            hparams.data_config = "test_no_overlap.yaml"
    else:
        if hparams.with_overlap:
            hparams.data_config = "val.yaml"
        else:
            hparams.data_config = "val_no_overlap.yaml"
    model_config = hparams.model_config
    data_config = hparams.data_config
    data_raw_parent_dir = hparams.data_raw_parent_dir

    model_path = OmegaConf.load(get_yaml("model", model_config))["ckpt"]
    if hparams.mode == "test":
        output_parent_dir = os.path.join(os.path.dirname(model_path), "output_images")
    else:
        output_parent_dir = os.path.join(os.path.dirname(model_path), "output_images_val")
    ...
    start_post_process(hparams.mode, names, data_config, model_config, complete_test_directory,
                       complete_postprocessed_directory, visualizations_dir, data_raw_parent_dir, data_parent_dir,
                       csv_file, experiment_name, on_cluster, load_results_from_file=load_results_from_file,
                       make_visualizations=True)
