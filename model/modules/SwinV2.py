...
"""
    A bunch of building Blocks for Swin V2
"""
...
class SwinTransformerBlock_Cross(nn.Module):
    ...
    def forward(self, x, y):
        H, W = self.input_resolution
        B, L, C = x.shape
        assert L == H * W, "input feature has wrong size expected " + str(H) + " x " + str(W) + " but got " + str(L)

        shortcut = x
        x = self.norm1(x)  # Norm happens later in Swin V2
        x = x.view(B, H, W, C)
        ...
        y_windows = y_windows.view(-1, self.window_size * self.window_size,
                                   C)
        # W-MSA/SW-MSA
        attn_windows = self.attn(x_windows, y_windows,
                                 mask=self.attn_mask)
        ...
        x = shortcut + self.drop_path(x)  # Norm1 Happens here now
        x = x + self.drop_path(self.norm2(self.mlp(x)))  # Changed to SwinV2 Format
